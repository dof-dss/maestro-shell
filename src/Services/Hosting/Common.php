  ...
  public function build($io) {
    ...
    foreach ($this->project()->sites() as $site_id => $site) {
      ...
      if (!$this->fs()->exists('/project/sites/' . $site_id)) {
        ...
        $this->fs()->copy($this->path() . '/files/multisite.settings.php', '/project/sites/' . $site_id . '/settings.php');
      }
    ...
    if (!$this->fs()->exists('/web/sites/services.yml')) {
      ...
      $this->fs()->copy($this->path() . '/files/default.services.yml', '/web/sites/services.yml');
    }
